...
[private vulnerability reporting](https://github.com/dean0x/croxy/security/advisories/new).
...
croxy is pre-1.0. Security fixes are applied to the latest release only; please
update to the newest commit before reporting.

## Security model

croxy is a **loopback-only development proxy**. It binds `127.0.0.1` and its
...
- **Loopback binding.** The server binds `127.0.0.1` and is never exposed to the
  network. Claude Code requires a plain-HTTP local base URL; traffic to croxy
  never leaves the machine, and both upstream legs use TLS.
- **Credentials never transit croxy's config.** The Anthropic leg is a verbatim
...
- **Bounded resources.** Request body size, SSE event size, the reasoning cache,
  and every timeout are explicitly bounded (see `croxy.config.example.json`).
- **No telemetry.** croxy makes no analytics or telemetry calls of its own.
