...
[![CI](https://github.com/dean0x/croxy/actions/workflows/ci.yml/badge.svg)](https://github.com/dean0x/croxy/actions/workflows/ci.yml)
...
croxy is a local subscription-routing proxy for Claude Code. Give a subagent a
...
your **claude.ai subscription**, untouched. No API keys — croxy forwards the
...
croxy is the first proxy that splits traffic **per subagent, by model name**:
...
```
Claude Code ──► croxy (127.0.0.1:4141)
...
```sh
git clone https://github.com/dean0x/croxy.git
cd croxy
npm install
...
Wire a project to croxy with `.claude/settings.json`:
...
sends it as `output_config.effort`, and croxy forwards it verbatim as Responses
...
and the backend default applies. When effort is forwarded, croxy logs
`codex_effort_applied`.

## Configuration

Optional configuration goes in `croxy.config.json` (gitignored). See
...
- **Auth**: reads `~/.codex/auth.json`, proactively refreshes the OAuth access
  ...
  newer file wins. On a 401 mid-flight, croxy force-refreshes and retries
  exactly once.
...
- Images in tool results are dropped on the Codex leg (logged as
  `image_dropped`).
- One croxy instance holds the reasoning cache in memory; restarting it
...
croxy is a loopback-only proxy that handles subscription credentials. Report
